...
cd open62541-compat
mkdir build && cd build
cmake -DSTANDALONE_BUILD=ON -DLOGIT_BUILD_OPTION=LOGIT_AS_EXT_STATIC -DOPEN62541-COMPAT_BUILD_CONFIG_FILE=boost_lcg.cmake -DLOGIT_INCLUDE_DIR=../../LogIt/include -DLOGIT_EXT_LIB_DIR=../../LogIt/build -DCMAKE_INSTALL_PREFIX=$DEPLOYMENT   ../ || exit
make || exit
make install || exit
cd $WD
}
